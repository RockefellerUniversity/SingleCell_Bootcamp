...
```{r sec3_loadPack,include=TRUE,eval=FALSE}
library(Seurat)
library(scran)
library(scater)
library(SeuratData)
...
---
## Detect TRUE cells by using Knee plot
...
---
## Detect empty droplets by using DropletUtils
...
## Correct ambient RNAs with DropletUtils

Lets start by estimating our ambient RNA contamination.
...
---
## Ambient RNA detection and correction by using SoupX
...
---
## Automatically estimate contamination and correction

The *autoEstCont()* can be used to detect contamination. We can then expor the correct counts with *adjustCount()*.
...
---
## Estimate contamination with mark gene sets
...
## Dealing with empty droplets and ambient RNAs by CellbBender

CellBender is a python toolkit developed to eliminate technical artifacts from scRNA-Seq data [link](https://cellbender.readthedocs.io/en/latest/introduction/index.html). It is run from the command line. 
...
## Load results from CellRanger and CellBender, respectively
```{r sec3_dropProc_cbFilt_loadData,include=TRUE,eval=T}
message("Filtered count matrix from CellRanger")
dim(filt_mtx)
#
message("Filtered count matrix from CellBender")
