...
Read in object after integration from dropbox 
...
```{r setup3,include=T,echo=T, fig.height=4}
...
"    
  ...
  cat("# Prepare object for differential analysis
...
# Differential expression using the Wilcoxon Rank Sum Test
...
"    
  ...
  cat("# Differential expression using the Wilcoxon Rank Sum Test
...
# Differential expression using parametric methods
...
"    
  ...
  cat("# Differential expression using parametric methods
...
# DELETE EVENTUALLY
# test various clusters for correlation with CDR
cells <- unique(seu_obj$paper_annot)
lapply(cells, function(x){
  seu_c2 <- subset(seu, paper_annot == x)
  sce_c2 <- as.SingleCellExperiment(seu_c2,assay = "RNA")
  rowData(sce_c2)$gene_id <- rownames(sce_c2)
  
  sca_c2 <- MAST::FromMatrix(exprsArray = as.matrix(logcounts(sce_c2)),
                             cData = as.data.frame(colData(sce_c2)),
                             fData = as.data.frame(rowData(sce_c2))
  )
  
  cdr <- colSums(assay(sca_c2)>0)
  colData(sca_c2)$ngeneson <- scale(cdr)
  cond <- factor(colData(sca_c2)$condition,
                 levels = c("AD","CON"))
  cond <- relevel(cond,"CON")
  colData(sca_c2)$Group <- cond
  
  pca_embed <- data.frame(seu_c2[["pca"]]@cell.embeddings)
  
  pca_vars <-  seu[["pca"]]@stdev^2 # variance for each pc
  total_var <- sum(pca_vars) # total variance
  pct_var_explained <- pca_vars/total_var * 100 
  pct_var_explained <- round(pct_var_explained, 1)
  
  for_plot <- data.frame(colData(sca_c2))
...
```{r}
mast_wilcox <- merge(de_mast_exn, de_wilcox, by = "geneID", suffixes = c("_mast", "_wilcox"))
de_compare <- table(mast_wilcox$sig_mast, mast_wilcox$sig_wilcox)
rownames(de_compare) <- c("mast_false", "mast_true")
colnames(de_compare) <- c("wilcox_false", "wilcox_true")
de_compare
...
```{r mast_violin1, echo=T, eval=T, fig.width=10}

VlnPlot(seu_exn, features = top_down_AD$geneID, stack = T, flip = T, pt.size = 1) + scale_x_discrete(labels=c('CON', 'AD'))
...
```{r mast_violin2, echo=T, eval=T, fig.width=10}

VlnPlot(seu_exn, features = sorta_down_AD$geneID, stack = T, flip = T, pt.size = 1) + scale_x_discrete(labels=c('CON', 'AD'))
...
.pull-right[
<img src="./imgs/pseudoreplication_bis_correlations.png"height="500" width="450">
]
...
```{r pseudo_plot,include=T,echo=T}
...
## MAST vs Pseudobulk
```{r}
mast_pseudo <- merge(de_mast_exn, de_pseudo_exn, by = "geneID", all = FALSE, suffixes = c("_mast", "_pseudo")) 
...
Look at the genes that go down in AD (up in control) in a feature plot
```{r pseudo_topSig, echo=T, eval=T, fig.width=10}
...
Look at the genes that go down in AD (up in control) with a violin plot
```{r pseudo_violin1, echo=T, eval=T, fig.width=10}
...
zlm_re <- zlm(~ngeneson + Group + (1 | sample_id),
               sca_exn_filt, 
               ebayes = FALSE, 
               method="glmer")
...
---
## Compare MAST results to Seurat wrapper 
