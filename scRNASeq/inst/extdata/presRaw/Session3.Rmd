...
```{r mergeData_woCor_merged,include=TRUE,eval=FALSE}
...
```{r sec3_mergeData_RPCA_cluster,include=TRUE,eval=T, echo=F}
...
```{r sec3_mergeData_MNN_cor,include=TRUE,eval=FALSE}
library(batchelor)
mnn_res <- fastMNN("CTRL"=sce_list$CTRL, "STIM"=sce_list$STIM,
                   d=50, # number of principle compoents used
                   k=20, # the number of nearest neighbors to consider
...
```{r sec3_ctAnno_Seurat_tranferAnno,include=TRUE,eval=FALSE}
...
```{r sec3_ctAnno_SingleR_import,include=TRUE,eval=FALSE}
...
```{r sec3_trajANA_trajSep,include=TRUE,eval=FALSE}
...
```{r sec3_trajANA_driveL1_topLate,include=TRUE,eval=F}
