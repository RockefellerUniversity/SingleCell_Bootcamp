...
This is Human PBMC data from 10X genomics. We will be using the filtered dataset that has been produced by Cell Ranger. 
...
```{r qcPlot_scatter_pres1,include=TRUE,eval=T, echo=F, fig.height=4,fig.width=7}
...
```{r qcPlot_scatter_pres2,include=TRUE,eval=F}
...
```{r qcPlot_ridgePlot_pres,include=TRUE,eval=F}
...
Though we have filtered out the most extreme problem cells we will lkely still have confounders in our dataset. These are unexpected variables contribute to a high percent of the variances.

- For SCTransform data, we can use the same parameter in SCTransform().
- Common potential confounders: percent.mt, doublet_score, cyclon_G1Score, cyclon_SScore, cyclon_G2MScore
...
```{r}
ggplot(deg, aes(x=avg_log2FC, y=-log10(p_val_adj), color=deg$stat)) + geom_point() +
