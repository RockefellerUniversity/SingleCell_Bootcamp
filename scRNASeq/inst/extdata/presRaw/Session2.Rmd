...
```{r ccPhase_comp,include=TRUE,eval=TRUE}
...
nFeature is bimodal
```{r qcPlot_vlnPlot_pres1,include=TRUE,eval=TRUE}
...
```{r evalVarExp,include=TRUE,eval=TRUE}
library(scater)
...
```{r scaleVar,include=TRUE,eval=TRUE}
...
```{r dimRed_elbow2_rep,include=TRUE,eval=T}
...
```{r clust_resoEval2,include=TRUE,eval=T}
...
```{r resEval_knownMarker,include=TRUE,eval=T}
